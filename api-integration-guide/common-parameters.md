...
| timestamp | STRING | Y | Timestamp, in UNIX time format (seconds), 10 digits long, current time - 300 seconds <= input time <= current time + 300 seconds |
| sign | STRING | Y | The API input parameter signature, the signature value is calculated according to the following algorithm |
| access_token | STRING | Y | User authorization token access_token , can be obtained from Seller Center, and the operation will issue the corresponding online store token |
